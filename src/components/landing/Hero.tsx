...
export function Hero() {
  ...
  return (
    <section className="relative pb-24 pt-0 lg:pb-32 lg:pt-8 xl:pb-40 xl:pt-12" style={{ background: "#050d0a" }}>
      ...
      {/* Navbar */}
        <nav className="relative z-20 max-w-5xl mx-auto px-5 h-16 flex items-center justify-between">
          <Link to="/" className="flex items-center gap-2.5">
            ...
            <span className="text-xl font-semibold text-white/90 tracking-tight">PT <span className="text-2xl font-bold">SS</span>N</span>
          </Link>
        ...
        <div className="hidden md:flex items-center gap-8 text-sm text-white">
          ...
          <a href="#why-us" className="hover:text-white/70 transition-colors">Why Us</a>
          <a href="#agency" className="hover:text-white/70 transition-colors">About</a>
          <a href="#reviews" className="hover:text-white/70 transition-colors">Clients</a>
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-5 pt-6 flex items-center gap-2">
        {HERO_IMAGES.map((_, i) => (
          <button
            key={i}
            onClick={() => setIdx(i)}
